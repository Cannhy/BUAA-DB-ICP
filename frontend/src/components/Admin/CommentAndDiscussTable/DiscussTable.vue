            ...
            <el-col :span="24">
              ...
              <el-card v-for="(postTheme, index) in showPostThemeList" :key="index"
                ...
                <div class="clearfix" style="font-size: 18px">
                  ...
                  <el-button style="float: right; padding: 3px 0;font-size: 17px;margin-top: 20px" type="text"
                ...
                <div class="textitem" style="margin-top: 2%; margin-bottom: 2%">
                  ...
                  <el-button style="float: right; padding: 3px 0;font-size: 17px; margin-top: 15px; color: red" type="text"
                             v-on:click="deletePostTheme(index)">删除帖子</el-button>
                </div>
                <div>
...
export default {
  ...
  components: {AdminMenu, quillEditor},
  data: function () {
    return {
      loading: true,
      userName: '',
  ...
  methods: {
    ...
    deletePostTheme: function (index) {
      this.$confirm('此操作将永久删除该主题贴及其跟贴，是否继续？', '提示', {
        ...
        this.$http.request({
          ...
          method: 'get',
          params: {
            mainId: that.showPostThemeList[index].id
          }
        }).then(function (response) {
          console.log(response.data)
          if (response.data === 0) {
